...
namespace alice{
  ...
  ALICE_COMMAND( read_lib, "STA", "Reads standard cell library"){
    ...
    sta->setTclInterp(_interp);
    std::string cornerName = "wst";
    sta::StringSet corner_set;
    corner_set.insert(cornerName.c_str());

    // sta->makeComponents();
    // std::cout << "check point 4\n";

    sta->makeCorners(&corner_set);

    std::cout << "Corner created\n";

    sta::Corner* corner = sta->findCorner(cornerName.c_str());
    std::cout << "Corner found\n";
    sta->readLiberty(filename.c_str(), corner, sta::MinMaxAll::min(), false);
    std::cout << "Liberty parsed\n";
  }
  ...
  class write_verilog_command : public alice::command{
    ...
    protected:
        ...
        if(checkExt(filename, "v")){
          if(is_set("mig")){
            if(!store<mockturtle::mig_network>().empty()){
              std::cout << "read from mig done " << std::endl; 
              auto& mig = store<mockturtle::mig_network>().current();
          ...
          if(is_set("xag")){
            if(!store<mockturtle::xag_network>().empty()){
              auto& xag = store<mockturtle::xag_network>().current();
              mockturtle::write_verilog(xag, filename);
            }
            else{
              std::cout << "There is not a XAG network stored.\n";
            }
          }
          else{
            if(!store<mockturtle::aig_network>().empty()){
...
ALICE_ADD_COMMAND(write_bench, "Output");
  ...
  class dump_npn_command : public alice::command{
    public:
      explicit dump_npn_command( const environment::ptr& env )
          : command( env, "Dumps a text file with all the NPN equivalent classes with up to 4 inputs" ){
      }

    protected:
      void execute(){
        /*
        - generates all the functions up to 4 inputs   
        - generates the logic representation for each function
        - switch flags for optimization (or not)
        - writes the output verilog
        ...
        myfile.close();
        */
        /* 4 input truth tables - need to update it to take a parameter */
        uint32_t num_vars = 4; 
        for (int i = 1; i <= num_vars; ++i)
        {
          ofstream db; 
          db.open ("npn_db.txt", std::ios_base::app);

          using truth_table = kitty::dynamic_truth_table;

          /* set to store all the 4-inputs NPN representatives */
  ...
  ALICE_ADD_COMMAND(dump_npn, "Output");

  class npn_choices_command : public alice::command{
    public:
      explicit npn_choices_command( const environment::ptr& env )
        : command( env, "Generates different combinations of AIGs to reduce structural bias." ){
        opts.add_option( "--filename,filename", filename, "Verilog file name to write out to" )->required();
        add_flag("--exact,-e",  "Runs exact optimization");
        add_flag("--recipe,-r", "Runs a resyn2-like recipe");
        add_flag("--combine,-c","Combine exact synthesis with the recipe");
        add_flag("--no,-n","Skip optimizations");
      }
    ...
    protected:
      void execute(){
        ...
        */
        ...
        std::cout << "Generated the resynthesis templates\n";

        auto mig = mockturtle::node_resynthesis<mockturtle::mig_network> ( aig, mig_resyn);
        auto xag = mockturtle::node_resynthesis<mockturtle::xag_network> ( aig, xag_resyn);
        auto xmg = mockturtle::node_resynthesis<mockturtle::xmg_network> ( aig, xmg_resyn);

        std::cout << "Derived new netorks\n";

        if(is_set("recipe")){
          //runs optimization recipes for each network 
          mockturtle::mig_script mig_opt;
          mockturtle::aig_script aig_opt; 
          mockturtle::xmg_script xmg_opt; 

          mig = mig_opt.run(mig);
          aig = aig_opt.run(aig);
          xmg = xmg_opt.run(xmg);
          
          std::cout << "Optimization completed\n Writing Verilog " + filename + " ...\n";
          auto tmp = filename + "_mig" + ".v";
          
          mockturtle::write_verilog( mig, tmp );
          tmp = filename;
          tmp = filename + "_aig" + ".v"; 
          mockturtle::write_verilog( aig, tmp );
          tmp = filename;
          tmp = filename + "_xmg" + ".v";
          mockturtle::write_verilog( xmg, tmp );
          ...
          //skip optimization recipes for each network
          std::cout << "No optimization to run\n";
        }
      }
    private:
      std::string filename{};
  ...
  ALICE_ADD_COMMAND(npn_choices, "Output");
  ...
  class seq_lut_map_command : public alice::command{
    public:
      explicit seq_lut_map_command( const environment::ptr& env )
          : command( env, "Runs LUT tech mapping" ){
      }
    ...
    protected:
      void execute(){
        ...
        std::cout << "AIG number of POs " << aig.num_pos() << std::endl; 
        ...
        mockturtle::write_bench(klut_topo, "lut_map.bench");
      }
  };

  ALICE_ADD_COMMAND(seq_lut_map, "Output");

  class read_db_command : public alice::command{

    public:
      explicit read_db_command( const environment::ptr& env )
          : command( env, "Reads json library for techmapping" ){

        opts.add_option( "--filename,-f", file, "JSON input file" )->required();
        opts.add_option( "--output,-o", output, "Output Verilog file" )->required();
        add_flag("--mig,-m", "Read from the stored MIG network");
    ...
    protected:
      void execute(){
        auto lib = oracle::read_npn_db (file);
        auto& aig = store<mockturtle::aig_network>().current();

        //it should not be here, just for testing! decouple classes
        if(is_set("mig"))
          auto& aig = store<mockturtle::mig_network>().current();
        ...
        std::cout << "Timing to create topo and mapping views " << duration.count() << std::endl;
        ...
        std::cout << "Timing to run LUT mapping " << duration.count() << std::endl;
        ...
        std::cout << "Timing to collapse LUT network " << duration.count() << std::endl;

        start = chrono::high_resolution_clock::now(); 
        auto const& klut = *klut_opt;
        mockturtle::topo_view klut_topo{klut};
        std::tuple<mockturtle::klut_network, std::unordered_map <int, std::string>> techmap_test = oracle::map_choices<mockturtle::klut_network>(klut_topo, lib, file); 
        
        oracle::write_techmapped_verilog(std::get<0>(techmap_test), output, std::get<1>(techmap_test), "test_top");
        end = chrono::high_resolution_clock::now(); 
        duration = chrono::duration_cast<chrono::microseconds>(end - start); 

        std::cout << "Timing to finish ASIC mapping " << duration.count() << std::endl;

        mockturtle::depth_view mapped_depth {std::get<0>(techmap_test)};
        std::cout << "\n\nDepth: " << mapped_depth.depth()<<"\n";
        
        mockturtle::write_bench(klut_topo, output + "KLUT.bench");
        mockturtle::write_bench(std::get<0>(techmap_test), output + "Techmapped.bench");

        auto end_all = chrono::high_resolution_clock::now(); 
        auto duration_tot = chrono::duration_cast<chrono::microseconds>(end_all - start_all); 

        std::cout << "Total running time " << duration_tot.count() << std::endl;
      }
    private:
      std::string file{};
      std::string output{};
  ...
  ALICE_ADD_COMMAND(read_db, "Input");

  class print_cuts_command : public alice::command{

    public:
      explicit print_cuts_command( const environment::ptr& env )
          : command( env, "Reads json library for techmapping" ){
        
        opts.add_option( "--number,-n", number, "Number of cuts to be stored per node. Default = 8" );
        opts.add_option( "--support,-k", size,  "Number of cut inputs. Default is k = 4" );
    ...
    protected:
      void execute(){
        ...
        const auto index = aig.node_to_index( n );
          std::cout << "Cuts for node " << aig.node_to_index( n ) << "\n";
        ...
        //rewrite graph just to get the chosen cut to rewrite 
        /* node resynthesis */
        mockturtle::xag_npn_resynthesis<mockturtle::aig_network> resyn;
        mockturtle::cut_rewriting_params stats;
        stats.cut_enumeration_ps.cut_size = size;
        stats.print_chosen = true;

        mockturtle::cut_rewriting(aig, resyn, stats);
        aig = mockturtle::cleanup_dangling(aig);

        std::string filename = aig._storage->net_name + "_lut.bench";
        mockturtle::mapping_view<mockturtle::aig_network, true> mapped{aig};

        mockturtle::lut_mapping_params map_ps;
        map_ps.cut_enumeration_ps.cut_size = size;
        map_ps.cut_enumeration_ps.cut_limit = number;

        mockturtle::lut_mapping<mockturtle::mapping_view<mockturtle::aig_network, true>, true>( mapped, map_ps );

        aig.foreach_node( [&]( auto n ) {
          if(mapped.is_cell_root(n)){
            std::cout << "Truth table for node " << aig.node_to_index(n) << " is " <<  kitty::to_hex(mapped.cell_function( n )) << std::endl;
          }
        } );
        ...
        mockturtle::write_bench( klut, "test_map.bench" );
      }
    private:
      uint32_t number = 3; 
      uint32_t size = 4;
  ...
  ALICE_ADD_COMMAND(print_cuts, "Input");
