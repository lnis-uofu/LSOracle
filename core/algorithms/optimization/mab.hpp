...
vector<string> aig_default_options()
{
    ...
    default_opts.push_back("rw");
    default_opts.push_back("rw -z");
    //default_opts.push_back("balance");
    default_opts.push_back("refactor -d");
    default_opts.push_back("refactor -z");
    default_opts.push_back("refactor -d -z");
    default_opts.push_back("resub -k 8");
    default_opts.push_back("resub -k 6 -s");
    default_opts.push_back("resub -k 6");
    default_opts.push_back("resub -k 8 -s");

    return default_opts;
}

/* TODO: lso-mig commands */
vector<string> mig_default_options()
{
    vector<string> default_opts;
    //default_opts.push_back("rw -m");
    default_opts.push_back("rw -m -a");
    default_opts.push_back("rw -m -z -a");
    //default_opts.push_back("balance");
    default_opts.push_back("refactor -m -d");
    default_opts.push_back("refactor -m -z");
    default_opts.push_back("refactor -m -d -z");
...
string clean_grep_2(string recmd)
{
    ...
    output.assign(temp, 15, temp.length()-2-15);
    return output;
}
...
string abc_stats_commmands(int h, int i, int which_opt)
{
    if (which_opt == 0 || which_opt == 1) //AIG minimization
        return "; ps -a;\" | grep \"nodes:\" | grep -v \"AIG\" > " + to_string(h)+"_"+to_string(i)+".result\n";
    else if (which_opt == 2 /*|| which_opt == 3*/) //MIG minimization
...
float get_area_from_result(string resultFile)
{
    ...
    find_area = input.find("nodes: ",0);
    int find_level = 0; find_level = input.find("level:",find_area);
    string area;
    if(find_area>0)
    {
...
float get_aig_size_from_result(string resultFile)
{
    ...
    int find_aig = input.find("nodes:", 0);
    int find_level = 0; find_level = input.find("level:",find_aig);
    string aig;
    //printf("get aig size c1 \n");
    if(find_aig>0)
    {
...
float get_aig_level_from_result(string resultFile)
{
    ...
    int find_lev = input.find("level:", 0);
    string aig;
    if(find_lev>0)
    {
