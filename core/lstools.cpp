          ...
          if(!store<oracle::partition_manager<mockturtle::aig_network>>().empty()){
            ...
            if(is_set("aig")){
              ...
              for(int i = 0; i < num_parts; i++){
                ...
                mig_opt_size = opt_mig.num_gates();
                mig_opt_depth = part_mig_opt_depth.depth();
                std::cout << "comparing " << i << "\n";
                std::cout << aig_opt_size << " " << aig_opt_depth << " " << mig_opt_size << " " << mig_opt_depth << '\n';
                #pragma opm critical(vector_push)
                {
           ...
           #pragma omp parallel for ordered
            for(int i = 0; i < aig_parts.size(); i++){
              ...
              mockturtle::depth_view opt_part_depth{opt};
               std::cout << "AIG part size after resynthesis = " << opt.num_gates() << " and depth = " << opt_part_depth.depth() << "\n";
              ...
              mockturtle::depth_view part_opt_depth{opt_mig};
               std::cout << "AIG new part size = " << opt_mig.num_gates() << " and depth = " << part_opt_depth.depth() << "\n";
