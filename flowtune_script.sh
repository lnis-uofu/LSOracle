...
# level-1 tuning
echo -e "ftune -d $ftune_input -r $repeat -t $target -p 1 -i $iteration -s $sample -L $liberty" | /mnt/nas/users/cam/LSOracle_mod/LSOracle/lib/FlowTune/FlowTune-AIG-Optimization/abc

## randomly pick one of the ftune script
shuf -n 1 $ftune_input.script > $design.script
echo -e "read $ftune_input; source $design.script;strash;write_aiger output_aig.aig;ps" | /mnt/nas/users/cam/LSOracle_mod/LSOracle/lib/FlowTune/FlowTune-AIG-Optimization/abc
...
do
	...
	echo -e "ftune -d /mnt/nas/users/cam/LSOracle_mod/LSOracle/benchmarks/output_aig.aig -r $repeat -t $target -p 1 -i $iteration -s $sample -L $liberty" | /mnt/nas/users/cam/LSOracle_mod/LSOracle/lib/FlowTune/FlowTune-AIG-Optimization/abc
	## randomly pick one of the ftune script
	shuf -n 1 internal.aig.script > $design.script
	#echo -e "read internal.aig; source $design.script;strash;write internal.aig" | ./abc
	echo -e "read /mnt/nas/users/cam/LSOracle_mod/LSOracle/benchmarks/output_aig.aig; source $design.script; write $design.ftune.v; strash;write_aiger /mnt/nas/users/cam/LSOracle_mod/LSOracle/benchmarks/output_aig.aig" | /mnt/nas/users/cam/LSOracle_mod/LSOracle/lib/FlowTune/FlowTune-AIG-Optimization/abc
done
