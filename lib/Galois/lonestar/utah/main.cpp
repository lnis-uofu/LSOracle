...
int main(int argc, char** argv) {
 ...
 // srand(-1);
  ...
  for (int level = 0; level<num; level++) {
    std::array<MetisGraph, k> metisGraph;
    ...
    for (auto h : graph.getNets()) {	
      ...
      int nPart = graph.getData(dst).getPart();
    ...
    for (auto i : toProcess) {
      ...
      for (auto c : gr->getNets()) {
        for (auto e : gr->edges(c)) 
          edges++;
      }
	  ...
	  for (auto n : gg.cellList()) {
            ...
            unsigned id = gg.getData(n).nodeid;
	     if (pp == 0) {
